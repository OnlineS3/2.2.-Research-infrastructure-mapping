 ...
 */
function getOptionsFor($dbname,$table,$column){
  $servername = "";
    $username = "";
    $password = "";
 ...
 */
function getQuery($filters,$table){
  if($table == 'esfris'){
    $query = "SELECT latitude, longitude, name, website , coordinating_country, headquarters , domain , location , partners , members , roadmap_entry , operation_start , preparation_cost , construction_cost , operation_cost , description , background , steps , activity , impact , esfri_type  FROM `$table`";
  }
  else{
    $query = "SELECT lat, lng, name, url , status , host , location , description  FROM `$table`";  
  }
